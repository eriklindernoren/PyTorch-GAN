...
class Logger():
    ...
    def log(self, losses=None, images=None, epoch=0, batch=0):
        ...
        if self.batches_done % self.sample_interval == 0:
            real = torch.cat((images['real_A'].data, images['real_B'].data), -1)
            fake = torch.cat((images['fake_B'].data, images['fake_A'].data), -1)
            recov = torch.cat((images['recov_A'].data, images['recov_B'].data), -1)
            save_image(torch.cat((real, fake, recov), -2),
